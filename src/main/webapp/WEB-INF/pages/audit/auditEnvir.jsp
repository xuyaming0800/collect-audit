...
<script type="text/javascript">
	...
	attrsMap,//存放任务明细,key为小区的一级,value为对应的明细
	imgsNo=0,//存放图片的key值,也是当前显示的第几张
	imgBeforeName,//图片所属的子任务名,记录当前点击的小区子任务的名称
	bpmTaskName,//当前任务所处于的节点名称
	bpmTaskId,//当前任务所处于的节点id
	submitJson={};//当前任务审核提交的内容
	$(function(){
		keysMap = new Map();
		//查询审核失败字典存储信息
		$.ajax({
		...
		function resizeBaiduMap() {
			var newHeight = $(window).innerHeight()-$("#allmap").offset().top-40;
		...
		map.addControl(new BMap.MapTypeControl({
			...
			offset: new BMap.Size(($("#allmap").width())/2, 45)}));   //添加地图类型控件
		...
		$.post(contextPath+"/audit/getObjectList.html",null,function(data){
			if(data!=null && data!=""){
				$.each(data.info.objectList,function(i,n){
					$('<li><a href="#" objectId="'+n.id+'">'+n.projectName+'</a></li>').appendTo("#getObjects");
				});
				//绑定事件
...
//					$("#taskType > li > a[id='frTask'] ").click();//未认领任务表格
					$("#taskType > li > a[id='fcTask'] ").click();//认领任务表格
					curr_taskId = "";//重置当前任务ID为空
					curr_proType = "";//重置当前任务类型
				});
			}
		...
		$("#taskType > li ").click(
			function() {
				...
				//选择发生变化则隐藏按钮
				$("#auditSuccessButton").hide();
				$("#auditOkButton").hide();
				$("#auditCancelButton").hide();
				$("#editLayerButton").hide();
			});
				...
				$.post(url,p,function(data){
					...
					if(data != null && data.length > 0){
						...
						$.each(data,function(i,n){
							...
							.click(function(e){
								if(submitJson.submit){
									if(!confirm("有审核的信息没有提交，确定要切换任务？")){
										return;
									}
								}
								submitJson={};
								...
								baseId = n.id;//任务id
								bpmTaskName = n.bpm_task_name;//节点名称
								bpmTaskId = n.taskDefinitionKey;//节点id
								//*************************
								curr_layerId = "";//清空图层ID
								curr_batchId = "";//清空子任务ID
								...
								function ctree(){
									$.post(contextPath+"/audit/getDetail.html",{
										...
										if(data!=""&&data!=null){
											...
											baseId = data.baseId;
											var showMoneyFlag = data.AUDIT_SHOW_MONEY;
											//将attrList[i].name  放入到显示中
											//attrs 遍历放入map中  点击获取 显示
											$("#audit_attrs_envir_div").empty();
											var label;
											for(var i=0;i<attrList.length;i++){
												if(attrList[i].status==0){
													label = "<div><label for='"+attrList[i].collectClazzName
														+"' onclick='showAttrEnvir(this,\""+attrList[i].batchId+"\");'> "+attrList[i].name
														+"</label><span onclick='changeStatus(\""+bpmTaskName+"\",\""+attrList[i].name+"\",0,"
														+attrList[i].money+",this);' class='glyphicon glyphicon-remove-sign' "
														+"aria-hidden='true' style='font-size: 20px; margin-left:10px;color:#D9534F'></span>";
												}else if(attrList[i].status==1){
													label = "<div><label for='"+attrList[i].collectClazzName
														+"' onclick='showAttrEnvir(this,\""+attrList[i].batchId+"\");'> "+attrList[i].name
														+"</label><span onclick='changeStatus(\""+bpmTaskName+"\",\""+attrList[i].name+"\",1,"
														+attrList[i].money+",this);' class='glyphicon glyphicon-ok-sign' "
														+"aria-hidden='true' style='font-size: 20px; margin-left:10px;color:#5CB85C'></span>";
												}else{
													label = "<div><label for='"+attrList[i].collectClazzName
														+"' onclick='showAttrEnvir(this,\""+attrList[i].batchId+"\");'> "+attrList[i].name
														+"</label><span onclick='changeStatus(\""+bpmTaskName+"\",\""+attrList[i].name+"\",0,"
														+attrList[i].money +",this);' class='glyphicon glyphicon-ok-sign' "
														+"aria-hidden='true' style='font-size: 20px; margin-left:10px;color:#5CB85C'></span><span onclick='changeStatus(\""
														+bpmTaskName+"\",\""+attrList[i].name+"\",1,"+attrList[i].money+",this);' "
														+"class='glyphicon glyphicon-remove-sign' aria-hidden='true' style='font-size: 20px; margin-left:10px;color:#D9534F'></span>";
												}
												//经办/抽检不显示
												if(bpmTaskId!="samplingUsertask"){
													label += "<span class='money_span'><button type='button' class='btn btn-danger btn-sm money_btn' onclick='changeMoney(\""+bpmTaskName+"\",\""
													+attrList[i].name+"\",this,"+attrList[i].money+");'>修改金额</button><input class='money_input' disabled='disabled' value=''></span></div>"
												}else label += "</div>";
												//大json
												submitJson[attrList[i].name]={};
												//把金额存入json，方便金额修改
												submitJson[attrList[i].name].money=
													attrList[i].userChangeMoney==null||attrList[i].userChangeMoney==undefined?attrList[i].money:attrList[i].userChangeMoney;
												submitJson[attrList[i].name].status=attrList[i].status;
												$("#audit_attrs_envir_div").append(label);
											}
											showLayerTypeDialog("#audit_attrs_envir");
											setAttrsShow(showMoneyFlag);//设置显示的树形属性项
										}else{
											$("#audit_attrs_envir_div").empty();
											$("#audit_attrs_envir").hide();
										}
									});
								}
								//***************************************************************************************************************
				...
				function findAround(photos,map,ooc,gisType,area,lon,lat){
					...
					if(ooc != null){
					 	for (coor in ooc) {
							...
							//obj.myAuditId;
							if(obj !=null){
								if(gisType == 1 || obj.length == 1 ){//点  -->20151110新增如果类型不为1，但是点为1那么也按照点来显示。
									var pointOther = new BMap.Point(obj[0].coordinateX, obj[0].coordinateY);
									 //设置marker图标为水滴
									var myIconOther = new BMap.Symbol(BMap_Symbol_SHAPE_POINT,{
									    scale: 0.6,//图标缩放大小
									    fillColor: "#6B0289",//填充颜色
...
//					$("#taskType > li > a[id='frTask'] ").click();//刷新未认领表格
					$("#taskType > li > a[id='fcTask'] ").click();//刷新认领表格
			});
		});
		//退领任务
...
//					$("#taskType > li > a[id='frTask'] ").click();//刷新未认领表格
		...
		$("#auditSuccessButton").click(function(){
			$("#auditSuccessButton").hide();
			$("#auditCancelButton").show().bind("click",function(){
				$("#auditOkButton").hide();
				$("#auditCancelButton").hide();
				$("#auditSuccessButton").show();
			});
			$("#auditOkButton").show();
			$("#auditOkButton").unbind().bind("click",function(){
				$.post(contextPath+"/audit/auditSuccess.html",
					...
					function(data){
						...
						if(data.flag==null)
							...
							submitJson = {};
							refresh();	
							$("#audit_attrs_envir_div").empty();//清空
							$("#audit_attrs_envir").hide();//隐藏
						}
						else if(data.flag==1){
							alert("审核通过完成");
							submitJson = {};
		...
		$("#searchButton").click(function(){
			...
			$("#searchCondition")
			.slideToggle()
			.offset({right:buttonOffset.left});
		});
		//日历
	...
	//显示明细
	function showAttrEnvir(op,batchId){
		curr_batchId = batchId;
		$("#audit_attrs_envir_div >div").each(function(i,n){
			$(this).removeClass("active")
		});
		//查询id
		$.post(
			contextPath+"/editPlatform/queryOneLayerByProType.html",{"pro_type":$(op).attr("for"),"pProTypeName":curr_proType,"rootId":tree_rootId},function(data){
				if(data.result=="success"){
					if(data.data)
						curr_layerId=data.data.id;
					else{
						alert("覆盖物有误，无法编辑");
						curr_layerId=null;
					}
		...
		var keyInAttrsMap = $(op).text().trim();
		imgBeforeName = keyInAttrsMap;
		//放入列表
		$("#ztreeContent").json2tree({
			json:attrsMap['container'][keyInAttrsMap],
			hides:"imgs",
			click: function(e){
			}
		});
	...
	//记录审核操作,信息放入json中
	function changeStatus(bname,name,status,money,op){
		if(taskTypeId=="fmiTask"){
			alert("经办任务无法修改状态");return;
		} else if(bname=="冻结期"){
			alert("冻结期无法修改状态");return;
		}
		if (taskTypeId == "fcTask"){//已认领任务
			submitJson[name]={};//清空对应name的审核信息
			submitJson.submit=true;//设置操作未提交标识
			//-----20151013获取修改金额[star]-----
			var changeMoney = $(op).nextAll("span[class='money_span']").children('.money_input').val();
			if(changeMoney) money = changeMoney;
			//-----20151013获取修改金额[end]------
			submitJson[name].money=money;
			//防止使用者点击审核不通过，然后直接关闭导致状态不完整的错误
			submitJson[name].status=1;
			if(status==1){
				//显示审核不通过弹窗
				$("#auditSuccessPanel").hide();
				toggleDialogLater("#auditFailurePanel");
				//绑定事件
				$("#doAuditFailureButton").unbind().bind("click",function(){
					submitJson[name].reason=$("#auditFailurePanel #noapprovalReason").find("option:selected").text();
					submitJson[name].messsage=$("#auditFailurePanel #commentMessage").val();
					submitJson[name].status=0;
					$("#auditFailurePanel").hide();
					$(op).next("span[class!='money_span']").remove();
					$(op).prev("span[class!='money_span']").remove();
					$(op).parent("div").append("<span onclick='changeStatus(\""+bname+"\",\""+name+"\",0,"+money
							+",this);' class='glyphicon glyphicon-remove-sign' aria-hidden='true' style='font-size: 20px; margin-left:10px;color:#D9534F'></span>");
					$(op).remove();
				});
			}else{
				//显示审核通过弹窗
				...
				$(op).next("span[class!='money_span']").remove();
				$(op).prev("span[class!='money_span']").remove();
				$(op).parent("div").append("<span onclick='changeStatus(\""+bname+"\",\""+name+"\",1,"+money
						+",this);' class='glyphicon glyphicon-ok-sign' aria-hidden='true' style='font-size: 20px; margin-left:10px;color:#5CB85C'></span>");
				$(op).remove();
			}
		}else alert("请先认领任务");
	}
	...
	function toggleDialogLater(el){
		...
		.offset({ 
			top: $("#allmap").offset().top+$("#allmap").height()-$("#auditFailurePanel").height()-5, 
			left: $("#allmap").offset().left+$("#allmap").width()-$("#auditFailurePanel").width()-$("#audit_attrs_envir").width()-5 
		});
	}
	
	//设置显示信息
	function setAttrsShow(op){
		for ( var p in attrs ){ 
			for(var m in attrs[p]){
				if(!op && (m=="userMoney"||m=="customMoney"||m=="money"||m=="userChangeMoney"))
					delete attrs[p][m];
				delete attrs[p]["money"];
			...
			attrsMap.put(p, attrs[p]);
		}
	}
	
	//图片翻页方法
	var myPosition_envir,circle_envir,marker_envir;
	function pageTurning(op,jump){
		if(imgBeforeName!=undefined&&imgBeforeName!=""){
			//删除之前的点
			map.removeOverlay(marker_envir);
			map.removeOverlay(circle_envir);
			map.removeOverlay(myPosition_envir);
			var imgs = attrs[imgBeforeName].imgs;
			if(imgs!=undefined && imgs.length > 0){//没有图片信息
				if(imgsNo + op<0){
					imgsNo = imgs.length-1;
				}else if(imgsNo + op>=imgs.length){
					imgsNo = 0;
				}else{
					imgsNo = imgsNo + op;
				}
				$("#imgsNo").text(imgsNo+1);
				//显示图片出来
				//economicEnvironment_showPic(attrs);
				//图片坐标定位
				if(imgs.length>0){
					point = new BMap.Point(imgs[imgsNo].lon, imgs[imgsNo].lat);
					//20151120禁止轮换图片导致地图定位
					if(jump) map.centerAndZoom(point, 18);  // 设置中心点坐标和地图级别
					map.enableScrollWheelZoom();
					circle_envir = new BMap.Circle(point,imgs[imgsNo].point_accury,{strokeColor:"#358ced", strokeWeight:2, strokeOpacity:0.1,fillColor:"#358ced", fillOpacity: 0.01}); //创建圆（经度圈）
					map.addOverlay(circle_envir);//增加圆
					//创建方向指示
					myPosition_envir = new SquareOverlay(point, 100, 100, null, function(div){
						return $(div).append($("<img id='positionImg' src='"+contextPath+"/images/position.png'/>").rotate({angle:parseInt(imgs[imgsNo].position)})).get(0);
					});
					...
					//显示图片
					if(imgs[imgsNo].image_url!=null&&imgs[imgsNo].image_url!=""){
						var newImgUrl = "/audit-watermark/collect-img/img";
						var list = (imgs[imgsNo].image_url).split("collect-img/img");
						newImgUrl = newImgUrl + list[1]
						...
						var _img = $('<div class="item text-center"><input name="isuse" type="hidden"/><input name="imgId" type="hidden"/><input name="index" type="hidden"/>'
								+'<img class="center-block img-responsive" style="cursor:pointer;" src="'+newImgUrl+'" onerror="javascript:this.src=\''+imgs[imgsNo].image_url+'\'"/>'
								+'<div class="carousel-caption">拍照时间:'+new Date(imgs[imgsNo].photograph_time*1000).Format("yyyy-MM-dd hh:mm:ss")
								+';卫星颗数:'+imgs[imgsNo].point_level
								+';拍摄方式:'+(imgs[imgsNo].index%2 == 1?"远景":"近景")+'</div>'
								+'</div>');
						//点击图片事件
						_img.click(function(){
							window.open(newImgUrl);
						});
						$("input[name=isuse]",_img).val(imgs[imgsNo].used);
						$("input[name=imgId]",_img).val(imgs[imgsNo].id);
						$("input[name=index]",_img).val(imgs[imgsNo].index);
						//插入图片
						$("#userPhotoOnePanel > .inner").append(_img);
						showDialogLater($("#userPhotoOnePanel"));
					}else{//显示摄影
						$('#userMp4Panel video#videoMp4').attr('src',imgs[imgsNo].video_url);
						$('#userMp4Panel a#videoMp4dow').attr('href',imgs[imgsNo].video_url);
						//缺失原因
						$('#userMp4Panel #noExistReason').html(imgs[imgsNo].no_exist_reason);
						showDialogLater("#userMp4Panel");
					}
				}
			}else {
				$("#userPhotoOnePanel").hide();//图片隐藏
			}
		}
	}
	function changeMoney(bname,name,op,oldMoney){
		$(op).hide();
		var inp = $(op).next(".money_input");
		$(inp).removeAttr("disabled").focus();
		$(".money_input").unbind().bind({
			blur:function(){ 
				setChangeMoneyInJson(bname,name,op,oldMoney);
				$(op).show();
	...
	//修改金额
	function setChangeMoneyInJson(bname,name,op,oldMoney){
		var changeMoney = $(op).next(".money_input").val();
		if(changeMoney){
			if(taskTypeId=="fmiTask"){
				alert("经办任务无法修改金额");$(op).next(".money_input").val("");return;
			} else if(bname=="冻结期"){
				alert("冻结期无法修改金额");$(op).next(".money_input").val("");return;
			}if (taskTypeId == "fcTask"){//已认领任务
				if(!/^[0-9]+(.[0-9]{1,2})?$/.test(changeMoney)){
					alert("请输入支持1-2位小数的正实数"); $(op).next(".money_input").val(""); return;
				}else if(parseFloat(oldMoney)<parseFloat(changeMoney)){
					alert("修改金额不能超过原始金额");$(op).next(".money_input").val("");return;
				}
				submitJson[name].money=changeMoney;
			}
		}
	}
</script>
...
<body>
		...
		<div class="row">
			<div class="col-xs-12 col-md-10">
				<div class="panel panel-default">
				  ...
				  <!-- 添加图层类型 -->
				  <!-- <div id="layerType" class="panel panel-default mydialog draggable" style="display:none;width:30%;overflow: auto;bottom:71px;right:26%;">
				...
				<!-- 任务详情的一级列表(环境数据) -->
				<div id="audit_attrs_envir" class="panel panel-default mydialog draggable" style="display:none;width:24%;overflow: auto;bottom:17px;right:31px;">
					<div class="panel-heading">子任务列表
						<button class="btn btn-default" id="turnLeft" onclick='pageTurning(-1);'><span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span></button>
						<span id="imgsNo" style="color:red;font-weight: bolder;">1</span>
						<button class="btn btn-default" id="turnRight" onclick='pageTurning(1);'><span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span></button>&nbsp;&nbsp;&nbsp;&nbsp;
						<button class="btn btn-primary" id="auditSuccessButton" style="display: none;"><span class="glyphicon glyphicon-thumbs-up" aria-hidden="true"></span>提交</button>
						<button class="btn btn-success" id="auditOkButton" style="display: none;"><span class="glyphicon glyphicon-ok-circle" aria-hidden="true"></span>确定</button>
						<button class="btn btn-inverse" id="auditCancelButton" style="display: none;"><span class="glyphicon glyphicon-remove-circle" aria-hidden="true"></span>取消</button>
						<button type="button" class="close_style_two">
							<span>&times;</span>
						</button>
					</div>
					<div style="height:250px;overflow-y:auto">
						<div id="audit_attrs_envir_div" style="margin-left: 20px;margin-top: 10px;"></div>
				  	</div>
				  	<div class="modal-footer" style="text-align:left;padding-left: 15px;padding-top: 2px;padding-bottom: 2px;" >
				  	     <div style="text-align:center;float:right;" >
							...
							<button class="btn btn-primary" id="editLayerButton">编辑</button>
				  	     </div>
				  	</div>
				</div>
					...
					<div class="panel panel-primary">
						<div class="panel-heading">
							<h3 class="panel-title">
								...
								<div id="searchCondition" style="margin-top: 12px; ">
									<form>
	...
	<!-- 审核不通过 -->
	<div id="auditFailurePanel" class="panel panel-default mydialog" style="display:none;width:25%;">
		...
		<div class="panel-body">
			...
			<div id="noapproval">
				...
				<button type="button" class="btn btn-primary"
					style="margin-top: 10px;float: right;" id="doAuditFailureButton"><span class="glyphicon glyphicon-thumbs-down" aria-hidden="true"></span>不通过</button>
			</div>
		</div>
	</div>
	<div id="ztree" class="panel panel-default mydialog draggable" style="display:none;width:20%;overflow: auto;height: 50%;">
		<div class="panel-heading">子任务明细
			<button type="button" class="close">
				<span>&times;</span>
			</button>
		</div>
	...
	<!-- <div id="doSwitch">
		<button class="btn btn-default" id="ensureSwitch" >确定</button>
		<button class="btn btn-default" id="cancelSwitch" >取消</button>
	</div> -->
</body>
</html>
