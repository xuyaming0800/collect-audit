		...
		function showTask(taskTypeId, cpage, isEmpty, callback) {
					...
					if(data != null && data.length > 0){
						...
						$.each(data,function(i,n){
							...
							.click(function(e){
								...
								//审核通过的道路赋值
								$("#audit_task_name").val(n.audit_task_name);
								//清空确认类型
			...
			function fullPhotoAndMarkMap(id,sysType){
						...
						$.each(data.userPhoto,function(i,n){
							...
							if(type == 7){
								...
								if(i==0){
									...
									$('#userMp4Panel .panel-body>div:last').clone().appendTo('#userMp4Panel .panel-body');
								...
								if(i==0) {
									...
									_addOverlay(false,data.userPhoto,map,centerPoint.lon,centerPoint.lat,null,n.position,oc,ooc,area,data.gisType);//如果是第一个，直接添加
								}
								slideMap.put(i+"",{map:map,lon:n.lon,lat:n.lat,point_accury:n.point_accury,position:n.position,oc:oc,gisType:data.gisType});//将事件加入map
							}else if(type == 9){
								//录像
								if(n.video_url&&n.video_url!=null && n.video_url!= ""){
									if(i==0){
										$('#userMp4Panel video#videoMp4').attr('src',n.video_url);
										$('#userMp4Panel a#videoMp4dow').attr('href',n.video_url);
										//缺失原因
				...
				function _addOverlay(flag,photos,map,lon,lat,point_accury,position,oc,ooc,area,gisType){
					...
					circle = new BMap.Circle(point,point_accury,{strokeColor:"#358ced", strokeWeight:2, strokeOpacity:0.5,fillColor:"#358ced", fillOpacity: 0.01}); //创建圆（经度圈）
					...
					geoc.getLocation(point, function(rs){
						...
						if(!$("#audit_task_name").val())
						$("#audit_task_name","#auditSuccessPanel").val(addComp.street);
						/* $("#consult,#audit_task_name","#auditSuccessPanel")
						.text(addComp.street)
						.click(function(){
							$(this).prev("input[id='audit_task_name']").val(addComp.street);
						})
						.css({
							"cursor":"pointer"
						}).hover(
				...
				function findAround(photos,map,ooc,gisType,area,lon,lat){
					...
					/* //画矩形
					var pStart = new BMap.Point(area.eMin,area.nMin);
					var pEnd = new BMap.Point(area.eMax,area.nMax);
						var rectangle = new BMap.Polygon([
						new BMap.Point(pStart.lng,pStart.lat),
						new BMap.Point(pEnd.lng,pStart.lat),
						new BMap.Point(pEnd.lng,pEnd.lat),
						new BMap.Point(pStart.lng,pEnd.lat)
					], {strokeColor:"blue", strokeWeight:1, strokeOpacity:0.01});//边色，边度，边透明度
					map.addOverlay(rectangle);//画多边形 */
				}
