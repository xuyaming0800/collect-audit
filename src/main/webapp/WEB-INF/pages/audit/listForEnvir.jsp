...
.pagination .controls input{
	border:0;color:#999;width:30px;padding:0;margin:-3px 0 0 0;text-align:center;
}
</style>
	...
	$(function(){
				...
				if(data!=null && data!=""){
					if(data.success){
						for(var i=0;i<data.info.length;i++){
							var n = data.info[i];
							var _div = $("<div class='newSel'>"+n.remark+"<div/></div>");
							$(_div).prependTo(_ap);
							$.ajax({
								...
								success: function(data1){
									if(data1!=null && data1!=""){
										if(data1.success){
											...
											$("div.newSel > div:empty:first",_ap).append(_sel);
											$.ajax({
		...
		$.post(contextPath+"/audit/getObjectList.html",null,function(data){
			if(data!=null && data!=""){
				...
				$("#audit_status li > a").click(function(){
					...
					auditStatus = self.attr("auditStatus");//项目ID
				})
		...
		$("#taskType > li ").click(
			function() {
				...
				cpage = 1;
				//showTask(taskTypeId, cpage, true, null);
				curr_taskId = "";//重置当前任务ID为空
			});
		...
		function showTask(taskTypeId, cpage, isEmpty, callback) {
			...
			fullDetail(contextPath+"/audit/auditList.html");
			...
			function fullDetail(url){//:visible
				...
				var p = {
		        	page: cpage-1,
		        	auditStatus:auditStatus==undefined?-1:auditStatus,
		        	bsType: bsType
				};
					...
					if(data != null && data.length > 0){
						...
						$.each(data,function(i,n){
							...
							.click(function(e){
								...
								//initData();//初始化覆盖物信息
								//显示图层类型窗口
								//$("#layerType").show().offset({ bottom: $("#allmap").offset().bottom, right: $("#allmap").offset().right });
								//**** 1,左下角列表-点击事件:显示树形结构
								//***************************************************************************************************************
										...
										if(data!=""){
											...
											for ( var p in attrs ){ 
												for(var m in attrs[p]){
													if(m=="collectClassId"||m=="batchId"||m=="reasons")
														delete attrs[p][m];
												...
												click: function(e){
													...
													if(t.parents("li.imgs").size()>0 && !t.parent("li").hasClass("imgs")){
														...
														myPosition_envir = new SquareOverlay(point, 100, 100, null, function(div){
															return $(div).append($("<img id='positionImg' src='"+contextPath+"/images/position.png'/>").rotate({angle:imgs[imgsIndex].position})).get(0);
														});
														...
														if(imgs[imgsIndex].image_url!=null&&imgs[imgsIndex].image_url!=""){
															...
															var _img = $('<div class="item text-center"><input name="isuse" type="hidden"/><input name="imgId" type="hidden"/><input name="index" type="hidden"/>'
																	...
																	+'<div class="carousel-caption">拍照时间:'+new Date(imgs[imgsIndex].photograph_time).Format("yyyy-MM-dd hh:mm:ss")
																			...
																			$.post(contextPath+"/audit/updateStatus.html",
																				{
																					...
																					occlusion:($("#auditSuccessPanel #dodging").val()),// 遮挡
					...
					if(data != null && data.length > 0){
					   ...
					   //initData();//初始化覆盖物数据
						...
						$.each(data.userPhoto,function(i,n){
							//轮播组件
							if(i==0) {
								_addOverlay(false,data.userPhoto,map,centerPoint.lon,centerPoint.lat,n.point_accury,n.position,oc,ooc,area,data.gisType,"#358ced");//如果是第一个，直接添加
							}
		...
		$("#doSearchButton").click(function(){
			...
			if(!bsType){
				...
				$.ajax({
					url: "<%=request.getContextPath()%>/audit/auditListCount.html",
					data:p,
					dataType: "json",
					success: function(data){
						data=$.parseJSON(data);
						if(data.count){
							 options.totalPages=data.count%data.pageSize==0?(data.count/data.pageSize):(data.count/data.pageSize+1);
		...
		<div class="row">
						...
						<div class="panel-body">
							...
							<form>查询条件：
								...
								<div class="btn-group">
									<button type="button" class="btn btn-default dropdown-toggle"
										data-toggle="dropdown" aria-expanded="false">
										<span id="audit_status_val">请选择审核状态</span><input id="auditStatus"
											type="hidden" ><span class="caret"></span>
									</button>
									<ul id="audit_status" class="dropdown-menu" role="menu">
										<li><a href="#">请选择审核状态</a></li>
										<li class="divider"></li>
										<li><a href="#" auditStatus="0">审核中</a></li>
										<li><a href="#" auditStatus="1">审核通过</a></li>
										<li><a href="#" auditStatus="2">审核未通过</a></li>
										<li><a href="#" auditStatus="3">申诉中</a></li>
									</ul>
								</div>
								<div class="form-group" style="display: initial;">
									<input type="text" class="form-control"
										id="collect_task_name" name="collect_task_name" placeholder="任务名" style="width: 20%;display:initial;">
								</div>		
								<div class="form-group" style="display: initial;">
									<input type="text" class="form-control"
										id="or	iginal_task_name" name="task_class_name" placeholder="类别" style="width: 20%;display:initial;">
								</div>		
								<div class="form-group" style="display: initial;">
									<input type="text" class="form-control"
										id="user_name" name="user_name" placeholder="用户名" style="width: 20%;display:initial;">
								</div>		
								<div class="text-center" style="display:initial;">
				...
				<div class="row">
					<div class="panel panel-primary">
						...
						<div class="panel-body">
							...
							<div><ul id="bp-3-element-lg" class="pagination pagination-lg"></ul></div>
						</div>
					</div>
				</div>
			</div>
			<div class="col-xs-12 col-md-10">
				<div class="panel panel-default">
				  ...
				  <!-- 添加图层类型 -->
				  <!-- <div>
					  <div id="layerType" class="panel panel-default mydialog draggable" style="display:none;width:30%;overflow: auto;bottom:71px;right:31px;">
						...
						<div style="height:100px;overflow-y:auto">
							<div id=" promptMsg" style="text-align: center;font-size: 15px;color: red;"> 请先添加图层类型</div>
							<div  id="typeDiv"  ></div>
					  	</div>
					  	<div class="modal-footer " style="text-align:left;" >
							 <div style="text-align:left;float:left;" >
					  	     ...
					  	     <div style="text-align:center;float:right;" >
					  	     	<button class="btn btn-info" type="button" id="addTypeButton" style="display: none;">
									添加类型
								</button>
								<button class="btn btn-info" type="button" id="editTypeButton" style="display: none;">
									编辑类型
								</button>
								<button class="btn btn-info" type="button" id="delTypeButton" style="display: none;">
									删除类型
								</button>
					  	     </div>
					  	</div>
